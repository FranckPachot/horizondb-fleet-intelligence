...
REPOSITORY_PATH = Path(__file__).parents[1] / "app" / "repository.py"
...
def test_schema_contains_horizon_embedding_contract() -> None:
    ...
    assert "current_position::public.geography" in schema
    assert "shipments_eta_idx" in schema
...
def test_repository_combines_postgis_diskann_and_hybrid_ranking() -> None:
    ...
    assert "0.28 * GREATEST" in repository
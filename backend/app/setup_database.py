...
def apply_schema(connection: psycopg.Connection[Any]) -> None:
    for extension in ("azure_ai", "vector", "pg_diskann", "postgis"):
        connection.execute(
...
def verify_embedding_model(
    ...
    with connection.cursor() as cursor:
        ...
        if current is None:
            raise RuntimeError(
                f"HorizonDB model alias {settings.embedding_model_alias!r} is not registered"
            )
    return True
...
def setup_database(
    ...
    *,
    ...
    with psycopg.connect(database_conninfo) as connection:
        ...
        model_registered = verify_embedding_model(connection, settings)
        if force_embeddings:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create and populate the Fleet Intelligence HorizonDB schema."
    )
    parser.add_argument(
        "--force-embeddings",
        action="store_true",
        help="Regenerate every HorizonDB model embedding before rebuilding DiskANN.",
    )
...
def main() -> None:
    ...
    print(
        f"Fleet Intelligence ready: {result.shipment_count} shipments, "
        f"{result.embedding_count} HorizonDB model embeddings, "
